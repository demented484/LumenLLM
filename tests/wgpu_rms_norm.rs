...
fn wgpu_rms_norm_matches_cpu_reference() {
    ...
    // GPU result
    let ctx = aegisllm::executor::wgpu::WgpuContext::new(0)
        .expect("failed to create WgpuContext");
    let gpu_out = aegisllm::executor::wgpu::rms_norm_gpu(&ctx, &input, &weight, eps)
        .expect("rms_norm_gpu failed");

    // CPU reference
    let mean_sq: f32 = input.iter().map(|v| v * v).sum::<f32>() / len as f32;
    let scale = 1.0 / (mean_sq + eps).sqrt();
    let cpu_out: Vec<f32> = input.iter().zip(weight.iter()).map(|(x, w)| x * scale * w).collect();
