...
pub(crate) fn prefill_prompt_token_by_token<B: GenerationBackendPrimitives + ?Sized>(
    backend: &B,
    state: &mut dyn GenerationState,
    prompt_tokens: &[usize],
...
pub(crate) fn sample_next_token(logits: &[f32], sampling: &SamplingConfig) -> Result<usize> {
    if logits.is_empty() {
