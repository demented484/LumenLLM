...
#[derive(Debug)]
pub(in crate::executor) struct CpuLayerBlockExecutor {
    hidden_size: usize,
...
impl CpuLayerBlockExecutor {
    pub(in crate::executor) fn from_artifact(
    ...
    pub(in crate::executor) fn new_state(&self) -> CpuLlamaState {
    ...
    pub(in crate::executor) fn embed_token(&self, token_id: usize) -> Result<Vec<f32>> {
        self.embed_tokens.row(token_id)
    }

    pub(in crate::executor) fn forward_layer_host(
        &self,
...
fn cpu_residency_for_store(store: StoragePlacement) -> crate::tensor::storage::TensorResidencyPlan {
    match store {
        StoragePlacement::Ram => crate::tensor::storage::TensorResidencyPlan::RamResident,
        StoragePlacement::Mmap | StoragePlacement::Vram { .. } => {
            crate::tensor::storage::TensorResidencyPlan::FileBackedMmap
        }
    }
}
