...
extern "C" __global__ void aegis_attention_prefill_paged_varlen_fa4_tile16(
    ...
    const unsigned int warp = tid >> 5u;
    const unsigned int nwarps = blockDim.x >> 5u;
    if (head >= num_attention_heads || global_q_base >= total_q || num_sequences != 1u || nwarps == 0u) {
        return;
    }
    ...
    float* q_shared = weights + q_block * k_tile;                 // q_block * head_dim
    float* k_shared = q_shared + q_block * head_dim;              // k_tile * head_dim
    float* v_shared = k_shared + k_tile * head_dim;               // k_tile * head_dim
    float* acc = v_shared + k_tile * head_dim;                    // q_block * head_dim
    float* scalars = acc + q_block * head_dim;                    // q_block * 4
    ...
    for (unsigned int row = 0u; row < q_block; ++row) {
        ...
        if (tid == 0u) {
            ...
            scalars[row * 4u + 2u] = 1.0f;            // alpha for current tile
            scalars[row * 4u + 3u] = 0.0f;            // unused reserve
        }
    }
    __syncthreads();
    ...
    for (unsigned int tile_start = 0u; tile_start < max_visible; tile_start += k_tile) {
        ...
        for (unsigned int idx = tid; idx < tile_count * head_dim; idx += blockDim.x) {
            const unsigned int col = idx / head_dim;
            const unsigned int dim = idx - col * head_dim;
            ...
            if (physical_slot < physical_slots) {
                const size_t kv_offset =
                    (size_t(physical_slot) * num_kv_heads + kv_head) * head_dim + dim;
                k_shared[col * head_dim + dim] = f16_bits_to_float(key_cache[kv_offset]);
                v_shared[col * head_dim + dim] = f16_bits_to_float(value_cache[kv_offset]);
            } else {
                k_shared[col * head_dim + dim] = 0.0f;
                v_shared[col * head_dim + dim] = 0.0f;
            }
        }
        __syncthreads();
        ...
        for (unsigned int row = 0u; row < q_block; ++row) {
            ...
            for (unsigned int col = 0u; col < k_tile; ++col) {
                ...
                if (col < tile_count && valid[row] && pos < visible_len[row]) {
                    for (unsigned int dim = tid; dim < head_dim; dim += blockDim.x) {
                        dot += q_shared[row * head_dim + dim] * k_shared[col * head_dim + dim];
                    }
                }
                #pragma unroll
                for (unsigned int offset = 16u; offset > 0u; offset >>= 1) {
        ...
        for (unsigned int row = 0u; row < q_block; ++row) {
            ...
            for (unsigned int dim = tid; dim < head_dim; dim += blockDim.x) {
                ...
                #pragma unroll
                for (unsigned int col = 0u; col < k_tile; ++col) {
                    tile_acc += weights[row * k_tile + col] * v_shared[col * head_dim + dim];
                }
                acc[row * head_dim + dim] = acc[row * head_dim + dim] * alpha + tile_acc;
            }
        }
        __syncthreads();
    }
