...
pub(crate) struct CudaKernelFunctions {
    ...
    pub(crate) fp8_block_matvec: CudaFunction,
    pub(crate) fp8_block_matmul_batched: CudaFunction,
    pub(crate) quantize_f32_to_fp8_token_group: CudaFunction,
    pub(crate) fp8_block_gemm: CudaFunction,
    ...
    pub(crate) bf16_matvec_warp: CudaFunction,
    ...
    pub(crate) gdn_qk_norm_expand: CudaFunction,
    pub(crate) gdn_gate: CudaFunction,
    pub(crate) gdn_gated_rmsnorm: CudaFunction,
    pub(crate) gdn_conv1d_decode: CudaFunction,
    // Batched (chunked-prefill) GDN kernels.
    pub(crate) gated_deltanet_prefill: CudaFunction,
    pub(crate) gdn_conv1d_prefill: CudaFunction,
    pub(crate) gdn_qk_norm_expand_batched: CudaFunction,
    pub(crate) gdn_gate_batched: CudaFunction,
    pub(crate) gdn_gated_rmsnorm_batched: CudaFunction,
    pub(crate) strided_copy_2d: CudaFunction,
    pub(crate) dequant_fp8_block_to_bf16: CudaFunction,
    pub(crate) deinterleave_gated_q: CudaFunction,
    pub(crate) sigmoid_gate_mul: CudaFunction,
    pub(crate) scale_by_sigmoid_scalar: CudaFunction,
...
impl CudaKernelFunctions {
    pub(crate) fn load(context: &Arc<CudaContext>, device_index: usize) -> Result<Self> {
        ...
        Ok(Self {
            ...
            fp8_block_matvec: load(&module, "aegis_fp8_block_matvec")?,
            fp8_block_matmul_batched: load(&module, "aegis_fp8_block_matmul_batched")?,
            quantize_f32_to_fp8_token_group: load(&module, "aegis_quantize_f32_to_fp8_token_group")?,
            fp8_block_gemm: load(&module, "aegis_fp8_block_gemm")?,
            ...
            bf16_matvec_warp: load(&module, "aegis_bf16_matvec_warp")?,
            ...
            gdn_qk_norm_expand: load(&module, "aegis_gdn_qk_norm_expand")?,
            gdn_gate: load(&module, "aegis_gdn_gate")?,
            gdn_gated_rmsnorm: load(&module, "aegis_gdn_gated_rmsnorm")?,
            gdn_conv1d_decode: load(&module, "aegis_gdn_conv1d_decode")?,
            gated_deltanet_prefill: load(&module, "aegis_gated_deltanet_prefill")?,
            gdn_conv1d_prefill: load(&module, "aegis_gdn_conv1d_prefill")?,
            gdn_qk_norm_expand_batched: load(&module, "aegis_gdn_qk_norm_expand_batched")?,
            gdn_gate_batched: load(&module, "aegis_gdn_gate_batched")?,
            gdn_gated_rmsnorm_batched: load(&module, "aegis_gdn_gated_rmsnorm_batched")?,
            strided_copy_2d: load(&module, "aegis_strided_copy_2d")?,
            dequant_fp8_block_to_bf16: load(&module, "aegis_dequant_fp8_block_to_bf16")?,
            deinterleave_gated_q: load(&module, "aegis_deinterleave_gated_q")?,
            sigmoid_gate_mul: load(&module, "aegis_sigmoid_gate_mul_f32")?,
            scale_by_sigmoid_scalar: load(&module, "aegis_scale_by_sigmoid_scalar_f32")?,
