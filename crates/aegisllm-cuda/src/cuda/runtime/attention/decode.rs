...
impl CudaRuntime {
    ...
    // -----------------------------------------------------------------------
    // Q8_0 KV attention decode (split-K, CUDA Graph friendly)
    // -----------------------------------------------------------------------
    /// Q8_0 KV FlashDecoding split-K. Quants (i8 as u8 byte) + per-32-element
    /// f16 scales. Combine pass is shared with the F16/FP8 paths.
    #[allow(clippy::too_many_arguments)]
    pub fn attention_decode_split_ptr_q8_0_device(
        &self,
        key_quants: &DeviceBuffer<u8>,
        value_quants: &DeviceBuffer<u8>,
        key_scales: &DeviceBuffer<u16>,
        value_scales: &DeviceBuffer<u16>,
        query: &DeviceBuffer<f32>,
        p_seq_len: &DeviceBuffer<u32>,
        num_attention_heads: usize,
        num_kv_heads: usize,
        ...
        if num_kv_heads == 0 || num_attention_heads == 0 || head_dim == 0 {
            return Err(AegisError::InvalidPlan(format!(
                "attention_split_q8_0: non-zero dims required q_heads={} kv_heads={} head_dim={}",
                num_attention_heads, num_kv_heads, head_dim
            )));
        }
        if !head_dim.is_multiple_of(32) {
            return Err(AegisError::InvalidPlan(format!(
                "attention_split_q8_0: head_dim={head_dim} not multiple of 32"
            )));
        }
        let query_len = checked_len("q8_0_split query", num_attention_heads, head_dim)?;
        if query.len() < query_len || output.len() < query_len {
            return Err(AegisError::InvalidPlan("attention_split_q8_0 query/output shape mismatch".into()));
        }
        let actual_split_k = seq_len_hint
            .div_ceil(DECODE_TARGET_CHUNK_LEN)
            .clamp(DECODE_SPLIT_K, DECODE_SPLIT_K_MAX);
        let partial_len     = checked_len("q8_0_split partial", num_attention_heads, DECODE_SPLIT_K_MAX)?;
        let partial_acc_len = checked_len("q8_0_split partial_acc", partial_len, head_dim)?;
        if partial_acc.len() < partial_acc_len || partial_m.len() < partial_len || partial_l.len() < partial_len {
            return Err(AegisError::InvalidPlan(format!(
                "attention_split_q8_0 partial buffers too small: acc={} m={} l={} req_acc={} req_ml={}",
                partial_acc.len(), partial_m.len(), partial_l.len(), partial_acc_len, partial_len
            )));
        }
        if !num_attention_heads.is_multiple_of(num_kv_heads) {
            return Err(AegisError::InvalidPlan("attention_split_q8_0 heads not divisible by kv heads".into()));
        }
        let num_heads_u32    = u32_arg("num_attention_heads", num_attention_heads)?;
        let num_kv_heads_u32 = u32_arg("num_kv_heads", num_kv_heads)?;
        let head_dim_u32     = u32_arg("head_dim", head_dim)?;
        ...
        let cache_capacity = key_quants.len() / kv_width;
        let cache_capacity_u32 = u32_arg("cache_capacity", cache_capacity)?;
        let block_dim        = CUDA_ATTENTION_BLOCK_DIM;
        let split_shared_bytes_usize =
            (effective_max_chunk_len + 4 + 4 * head_dim) * std::mem::size_of::<f32>();
        ...
        // Validate scale-buffer length: capacity * num_kv_heads * (head_dim/32)
        let blocks_per_head = head_dim / 32;
        let expected_scales = cache_capacity * num_kv_heads * blocks_per_head;
        if key_scales.len() < expected_scales || value_scales.len() < expected_scales {
            return Err(AegisError::InvalidPlan(format!(
                "attention_split_q8_0 scale buffers too small: key={} value={} expect={}",
                key_scales.len(), value_scales.len(), expected_scales
            )));
        }
        unsafe {
            self.stream
                .launch_builder(&self.kernels.attention_decode_ptr_split_q8_0)
                .arg(&key_quants.slice)
                .arg(&value_quants.slice)
                .arg(&key_scales.slice)
                .arg(&value_scales.slice)
                .arg(&query.slice)
                .arg(&p_seq_len.slice)
                .arg(&num_heads_u32)
                .arg(&num_kv_heads_u32)
        ...
        .map_err(map_cuda_err("launch attention_decode_ptr_split_q8_0"))?;
        ...
        .map_err(map_cuda_err("launch attention_decode_ptr_combine (q8_0)"))?;
        Ok(())
    }

    /// K8V16 hybrid decode: Q8_0 K + F16 V. Preserves V cancellation while
    /// saving ~23% KV memory vs all-f16. See `kv_q8_0.cu` for kernel details.
    #[allow(clippy::too_many_arguments)]
    pub fn attention_decode_split_ptr_k8_v16_device(
        &self,
        key_quants: &DeviceBuffer<u8>,
        key_scales: &DeviceBuffer<u16>,
        value_cache: &DeviceBuffer<u16>,
        ...
        if num_kv_heads == 0 || num_attention_heads == 0 || head_dim == 0 {
            return Err(AegisError::InvalidPlan(format!(
                "k8v16: non-zero dims required q_heads={} kv_heads={} head_dim={}",
                num_attention_heads, num_kv_heads, head_dim
            )));
        }
        if !head_dim.is_multiple_of(32) {
            return Err(AegisError::InvalidPlan(format!(
                "k8v16: head_dim={head_dim} not multiple of 32"
            )));
        }
        let query_len = checked_len("k8v16 query", num_attention_heads, head_dim)?;
        if query.len() < query_len || output.len() < query_len {
            return Err(AegisError::InvalidPlan("k8v16 query/output shape mismatch".into()));
        }
        let actual_split_k = seq_len_hint
            .div_ceil(DECODE_TARGET_CHUNK_LEN)
            .clamp(DECODE_SPLIT_K, DECODE_SPLIT_K_MAX);
        let partial_len     = checked_len("k8v16 partial", num_attention_heads, DECODE_SPLIT_K_MAX)?;
        let partial_acc_len = checked_len("k8v16 partial_acc", partial_len, head_dim)?;
        if partial_acc.len() < partial_acc_len || partial_m.len() < partial_len || partial_l.len() < partial_len {
            return Err(AegisError::InvalidPlan(format!(
                "k8v16 partial buffers too small"
            )));
        }
        if !num_attention_heads.is_multiple_of(num_kv_heads) {
            return Err(AegisError::InvalidPlan("k8v16 heads not divisible by kv heads".into()));
        }
        let num_heads_u32    = u32_arg("num_attention_heads", num_attention_heads)?;
        let num_kv_heads_u32 = u32_arg("num_kv_heads", num_kv_heads)?;
        let head_dim_u32     = u32_arg("head_dim", head_dim)?;
        ...
        let blocks_per_head = head_dim / 32;
        let expected_scales = cache_capacity * num_kv_heads * blocks_per_head;
        if key_scales.len() < expected_scales {
            return Err(AegisError::InvalidPlan(format!(
                "k8v16 scale buffer too small: key={} expect={}",
                key_scales.len(), expected_scales
            )));
        }
        if value_cache.len() < cache_capacity * kv_width {
            return Err(AegisError::InvalidPlan(format!(
                "k8v16 value cache too small: v={} expect={}",
                value_cache.len(), cache_capacity * kv_width
            )));
        }
        unsafe {
            self.stream
                .launch_builder(&self.kernels.attention_decode_ptr_split_k8_v16)
                .arg(&key_quants.slice)
                .arg(&value_cache.slice)
                .arg(&key_scales.slice)
                .arg(&query.slice)
        ...
        .map_err(map_cuda_err("launch attention_decode_ptr_split_k8_v16"))?;
        ...
        .map_err(map_cuda_err("launch attention_decode_ptr_combine (k8v16)"))?;
        Ok(())
    }
