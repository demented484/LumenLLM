...
impl CudaRuntime {
    ...
    // -----------------------------------------------------------------------
    // Q8_0 KV store methods (32-element blocks, int8 quants + f16 scales).
    // -----------------------------------------------------------------------
    // Separate quants (i8) and scales (u16 = f16 bits) buffers. head_dim must
    // be a multiple of 32. See `kv_q8_0.cu` for layout details.

    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_q8_0_ptr_device(
        &self,
        key_quants: &mut DeviceBuffer<u8>,
        value_quants: &mut DeviceBuffer<u8>,
        key_scales: &mut DeviceBuffer<u16>,
        value_scales: &mut DeviceBuffer<u16>,
        key: &DeviceBuffer<f32>,
        value: &DeviceBuffer<f32>,
        p_position: &DeviceBuffer<u32>,
        kv_width: usize,
        ...
        const QK: usize = 32;
        if !kv_width.is_multiple_of(QK) {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_ptr: kv_width={kv_width} not multiple of {QK}"
            )));
        }
        if key.len() < kv_width || value.len() < kv_width {
            return Err(AegisError::InvalidPlan("kv_store_q8_0_ptr vector shape mismatch".into()));
        }
        if key_quants.len() != value_quants.len()
            || key_quants.len() % kv_width != 0
            || key_quants.is_empty()
        {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_ptr quant cache shape: key={} value={} width={}",
                key_quants.len(), value_quants.len(), kv_width
            )));
        }
        let blocks_per_slot = kv_width / QK;
        let expect_scale_len = (key_quants.len() / kv_width) * blocks_per_slot;
        if key_scales.len() != expect_scale_len || value_scales.len() != expect_scale_len {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_ptr scale shape: key_scales={} value_scales={} expect={}",
                key_scales.len(), value_scales.len(), expect_scale_len
            )));
        }
        let width_u32 = u32_arg("kv_width", kv_width)?;
        let cap_u32 = u32_arg("cache_capacity", context_size)?;
        let num_blocks_u32 = u32_arg("num_blocks_per_slot", blocks_per_slot)?;
        let cfg = LaunchConfig {
            grid_dim: (ceil_div(num_blocks_u32, 64), 1, 1),
            block_dim: (64, 1, 1),
            shared_mem_bytes: 0,
        };
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_q8_0_ptr)
                .arg(&mut key_quants.slice)
                .arg(&mut value_quants.slice)
                .arg(&mut key_scales.slice)
                .arg(&mut value_scales.slice)
                .arg(&key.slice)
                .arg(&value.slice)
                .arg(&p_position.slice)
                .arg(&width_u32)
        ...
        .map_err(map_cuda_err("launch kv_store_q8_0_ptr"))?;
        Ok(())
    }

    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_q8_0_slots_batched_device(
        &self,
        key_quants: &mut DeviceBuffer<u8>,
        value_quants: &mut DeviceBuffer<u8>,
        key_scales: &mut DeviceBuffer<u16>,
        ...
        if !kv_width.is_multiple_of(QK) {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_slots: kv_width={kv_width} not multiple of {QK}"
            )));
        }
        let vector_len = checked_len("q8_0 slot kv", batch, kv_width)?;
        let cache_len  = checked_len("q8_0 slot cache", context_size, kv_width)?;
        let blocks_per_slot = kv_width / QK;
        let scale_cache_len = checked_len("q8_0 slot scales", context_size, blocks_per_slot)?;
        if dense_metadata.batch() != batch || dense_metadata.context_len() > context_size {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_slots: proof mismatch batch={} ctx={}",
                batch, context_size
            )));
        }
        if key.len() < vector_len || value.len() < vector_len {
            return Err(AegisError::InvalidPlan("kv_store_q8_0_slots vector shape mismatch".into()));
        }
        if slot_mapping.len() < batch
            || key_quants.len() != cache_len
            || value_quants.len() != cache_len
            || key_scales.len() != scale_cache_len
            || value_scales.len() != scale_cache_len
        {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_slots cache mismatch: kq={} vq={} ks={} vs={} expect_kv={} expect_scale={}",
                key_quants.len(), value_quants.len(), key_scales.len(), value_scales.len(),
                cache_len, scale_cache_len
            )));
        }
        let batch_u32    = u32_arg("batch", batch)?;
        let width_u32    = u32_arg("kv_width", kv_width)?;
        let context_u32  = u32_arg("context_size", context_size)?;
        let num_blocks_u32 = u32_arg("num_blocks_per_slot", blocks_per_slot)?;
        let cfg = LaunchConfig {
            grid_dim: (ceil_div(num_blocks_u32, 64), batch_u32, 1),
            block_dim: (64, 1, 1),
            shared_mem_bytes: 0,
        };
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_q8_0_slots_batched)
                .arg(&mut key_quants.slice)
                .arg(&mut value_quants.slice)
                .arg(&mut key_scales.slice)
                .arg(&mut value_scales.slice)
        ...
        .map_err(map_cuda_err("launch kv_store_q8_0_slots_batched"))?;
        Ok(())
    }

    // K-only Q8_0 stores (K8V16 hybrid path).
    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_q8_0_k_only_ptr_device(
        &self,
        key_quants: &mut DeviceBuffer<u8>,
        key_scales: &mut DeviceBuffer<u16>,
        key: &DeviceBuffer<f32>,
        ...
        if !kv_width.is_multiple_of(QK) {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_ptr: kv_width={kv_width} not multiple of {QK}"
            )));
        }
        if key.len() < kv_width {
            return Err(AegisError::InvalidPlan("kv_store_q8_0_k_only_ptr vector shape mismatch".into()));
        }
        if key_quants.len() % kv_width != 0 || key_quants.is_empty() {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_ptr quant cache shape: key={} width={}",
                key_quants.len(), kv_width
            )));
        }
        let blocks_per_slot = kv_width / QK;
        let expect_scale_len = (key_quants.len() / kv_width) * blocks_per_slot;
        if key_scales.len() != expect_scale_len {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_ptr scale shape: key_scales={} expect={}",
                key_scales.len(), expect_scale_len
            )));
        }
        let width_u32 = u32_arg("kv_width", kv_width)?;
        let cap_u32 = u32_arg("cache_capacity", context_size)?;
        ...
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_q8_0_k_only_ptr)
                .arg(&mut key_quants.slice)
                .arg(&mut key_scales.slice)
                .arg(&key.slice)
                .arg(&p_position.slice)
        ...
        .map_err(map_cuda_err("launch kv_store_q8_0_k_only_ptr"))?;
        Ok(())
    }

    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_q8_0_k_only_slots_batched_device(
        &self,
        key_quants: &mut DeviceBuffer<u8>,
        key_scales: &mut DeviceBuffer<u16>,
        key: &DeviceBuffer<f32>,
        ...
        if !kv_width.is_multiple_of(QK) {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_slots: kv_width={kv_width} not multiple of {QK}"
            )));
        }
        let vector_len = checked_len("q8_0 k_only slot kv", batch, kv_width)?;
        let cache_len  = checked_len("q8_0 k_only slot cache", context_size, kv_width)?;
        let blocks_per_slot = kv_width / QK;
        let scale_cache_len = checked_len("q8_0 k_only slot scales", context_size, blocks_per_slot)?;
        if dense_metadata.batch() != batch || dense_metadata.context_len() > context_size {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_slots: proof mismatch batch={} ctx={}",
                batch, context_size
            )));
        }
        if key.len() < vector_len {
            return Err(AegisError::InvalidPlan("kv_store_q8_0_k_only_slots vector shape mismatch".into()));
        }
        if slot_mapping.len() < batch
            || key_quants.len() != cache_len
            || key_scales.len() != scale_cache_len
        {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_q8_0_k_only_slots cache mismatch"
            )));
        }
        let batch_u32    = u32_arg("batch", batch)?;
        let width_u32    = u32_arg("kv_width", kv_width)?;
        ...
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_q8_0_k_only_slots_batched)
                .arg(&mut key_quants.slice)
                .arg(&mut key_scales.slice)
                .arg(&key.slice)
                .arg(&slot_mapping.slice)
        ...
        .map_err(map_cuda_err("launch kv_store_q8_0_k_only_slots_batched"))?;
        Ok(())
    }

    /// V-only f16 store (used in K8V16 hybrid path).
    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_f16_v_only_ptr_device(
        &self,
        value_cache: &mut DeviceBuffer<u16>,
        value: &DeviceBuffer<f32>,
        p_position: &DeviceBuffer<u32>,
        ...
        if value.len() < kv_width {
            return Err(AegisError::InvalidPlan("kv_store_f16_v_only vector shape mismatch".into()));
        }
        if value_cache.len() % kv_width != 0 || value_cache.is_empty() {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_f16_v_only cache shape: v={} width={}",
                value_cache.len(), kv_width
            )));
        }
        let width_u32 = u32_arg("kv_width", kv_width)?;
        let cap_u32 = u32_arg("cache_capacity", context_size)?;
        let cfg = LaunchConfig {
            grid_dim: (ceil_div(width_u32, 256), 1, 1),
            block_dim: (256, 1, 1),
            shared_mem_bytes: 0,
        };
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_f16_v_only_ptr)
                .arg(&mut value_cache.slice)
                .arg(&value.slice)
                .arg(&p_position.slice)
                .arg(&width_u32)
        ...
        .map_err(map_cuda_err("launch kv_store_f16_v_only_ptr"))?;
        Ok(())
    }

    #[allow(clippy::too_many_arguments)]
    pub fn store_kv_f16_v_only_slots_batched_device(
        &self,
        value_cache: &mut DeviceBuffer<u16>,
        value: &DeviceBuffer<f32>,
        slot_mapping: &DeviceBuffer<u32>,
        ...
        let vector_len = checked_len("f16 v slot kv", batch, kv_width)?;
        let cache_len  = checked_len("f16 v slot cache", context_size, kv_width)?;
        if dense_metadata.batch() != batch || dense_metadata.context_len() > context_size {
            return Err(AegisError::InvalidPlan(format!(
                "kv_store_f16_v_only_slots: proof mismatch batch={} ctx={}",
                batch, context_size
            )));
        }
        if value.len() < vector_len {
            return Err(AegisError::InvalidPlan("kv_store_f16_v_only_slots vector shape mismatch".into()));
        }
        if slot_mapping.len() < batch || value_cache.len() != cache_len {
            return Err(AegisError::InvalidPlan("kv_store_f16_v_only_slots cache mismatch".into()));
        }
        let batch_u32    = u32_arg("batch", batch)?;
        let width_u32    = u32_arg("kv_width", kv_width)?;
        let context_u32  = u32_arg("context_size", context_size)?;
        let cfg = LaunchConfig {
            grid_dim: (ceil_div(width_u32, 256), batch_u32, 1),
            block_dim: (256, 1, 1),
            shared_mem_bytes: 0,
        };
        unsafe {
            self.stream
                .launch_builder(&self.kernels.kv_store_f16_v_only_slots_batched)
                .arg(&mut value_cache.slice)
                .arg(&value.slice)
                .arg(&slot_mapping.slice)
                .arg(&batch_u32)
        ...
        .map_err(map_cuda_err("launch kv_store_f16_v_only_slots_batched"))?;
        Ok(())
    }
