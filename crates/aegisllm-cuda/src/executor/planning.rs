...
pub(super) fn cuda_limitations(
    ...
    match placement.kv_cache.quantization {
        ...
        KvCacheQuantization::Fp8 => limitations.push(
            "kv-cache fp8 has decode kernels but prefill attention does not yet read FP8 \
             KV — end-to-end FP8 KV is gated until the prefill kernel lands. Use f16 or bf16 \
             until then.".into(),
        ),
