...
pub fn rms_norm_gpu(
    ...
    rx.recv()
        .map_err(|_| aegisllm_base::error::AegisError::Unsupported("wgpu readback: channel closed".into()))?
        .map_err(|e| aegisllm_base::error::AegisError::Unsupported(format!("wgpu map_async failed: {e:?}")))?;
