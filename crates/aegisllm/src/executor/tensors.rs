...
impl Bf16Matrix {
    pub(crate) fn from_named_tensor(
    ...
    pub(crate) fn from_first_existing_tensor(
        artifact: &ModelArtifact,
        names: &[&str],
        store: StoragePlacement,
        loader: &mut TensorStorageLoader,
    ...
    pub(crate) fn row(&self, row: usize) -> Result<Vec<f32>> {
        if row >= self.rows {
            return Err(AegisError::InvalidPlan(format!(
    ...
    pub(crate) fn matvec_into(&self, input: &[f32], output: &mut [f32]) -> Result<()> {
...
pub(crate) fn read_dense_vector(
    tensor: &TensorInfo,
    store: StoragePlacement,
    loader: &mut TensorStorageLoader,
) -> Result<Vec<f32>> {
